...
-- AUTO CLICK SYSTEM - Ẩn và tự động kích hoạt
...
Tabs.framboss:AddToggle("AutoFarm", { Title = "Farm Boss", Default = false }):OnChanged(function(val)
    ...
    if val then 
        ...
        if autoClickEnabled then
            ...
            Fluent:Notify({
                Title = "Auto Farm",
                Content = "Đã tự động tắt Auto Click",
                Duration = 3
            })
        end
    end
...
function SafeCircleAroundBoss(bossRoot)
    task.spawn(function()
        ...
        while AutoFarm and bossRoot.Parent and bossRoot.Parent:FindFirstChild("Humanoid") and bossRoot.Parent.Humanoid.Health > 0 do
            ...
            -- Auto click ngẫu nhiên (đã được bật tự động)
...
task.spawn(function()
    ...
    Fluent:Notify({
        ...
        Content = "Auto Click & Auto Heal sẽ tự động bật/tắt khi farm boss",
        Duration = 5
    })
end)
