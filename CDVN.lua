...
-- Auto Băng Gạc được tối ưu để không lag server
task.spawn(function()
    while task.wait(2) do -- Giảm tần suất kiểm tra
        if AutoBangGac and CanUseBandage then
            ...
            if humanoid and humanoid.Health < 80 then
                ...
                if tool then
                    ...
                    task.wait(0.5) -- Giảm tốc độ
                    tool:Activate()
                end
                repeat task.wait(2) until (humanoid.Health > 80) or (not AutoBangGac) -- Tăng thời gian chờ
                task.wait(3)
                CanUseBandage = true
            end
        end
...
Tabs.Main:AddToggle("AutoBuyBandage", {
    Title = "Auto Mua Băng Gạc (10s)",
    Default = false,
    Callback = function(state) AutoBuyBandage = state end
})

task.spawn(function()
    while task.wait(10) do -- Tăng thời gian chờ lên 10s
...
task.spawn(function()
    while task.wait(2) do -- Giảm tần suất cập nhật
...
-- Auto Equip Weapon được tối ưu
task.spawn(function()
    while task.wait(3) do  -- Tăng thời gian chờ lên 3 giây
        if AttackWeapon and canEquip then
            ...
            task.wait(2)  -- Giảm thời gian chờ
            canEquip = true
        end
    end
end)
...
function StartAutoLoot()
    task.spawn(function()
        while AutoLoot do
            task.wait(1) -- Tăng thời gian chờ
...
-- Auto Farm được tối ưu để không lag server
        ...
        while AutoFarm do
            ...
            if boss then
                ...
                while AutoFarm and boss and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 do
                    task.wait(0.5) -- Thêm thời gian chờ để giảm lag
                end

                task.wait(5)
            else
                task.wait(1) -- Thêm thời gian chờ khi không có boss
            end
        end
    end)
end
...
function CircleAroundBoss(bossRoot)
    task.spawn(function()
        ...
        while AutoFarm and bossRoot.Parent and bossRoot.Parent:FindFirstChild("Humanoid") and bossRoot.Parent.Humanoid.Health > 0 do
            angle = angle + speed * task.wait(0.05) -- Giảm tốc độ cập nhật
...
task.spawn(function()
	...
	while task.wait(1500) do -- Tăng thời gian chờ lên 25 phút
		vu:Button2Down(Vector2.new(0, 0), workspace.CurrentCamera.CFrame)
		task.wait(0.5)
		vu:Button2Up(Vector2.new(0, 0), workspace.CurrentCamera.CFrame)
	end
...
-- Thêm nút bật/tắt menu
local menuVisible = true
local menuToggleBtn = Tabs.Main:AddButton({
    Title = "Ẩn Menu",
    Callback = function()
        menuVisible = not menuVisible
        if menuVisible then
            Window:Show()
            menuToggleBtn:SetTitle("Ẩn Menu")
        else
            Window:Hide()
            menuToggleBtn:SetTitle("Hiện Menu")
        end
    end
})
